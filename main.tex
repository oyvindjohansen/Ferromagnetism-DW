...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=1.0\linewidth]{Figures/walkerBreakdownv2}
  \caption{}
  \label{fig:walkerBreakdown}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=1.0\linewidth]{Figures/criticalCurrentXdotv2}
  \caption{}
  \label{fig:criticalCurrent}
\end{subfigure}
\caption{The translational speed of the domain wall with a high damping parameter $\xi$ of the itinerant electrons (a) and with a small damping of the itinerant electrons (b) as a function of applied current. The absolute value of $\langle\dot{X}\rangle$ is shown in the plot, the true value is negative.}
\label{fig:Xdot_afo_current}
\end{figure}
\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.8\textwidth]{Figures/criticalCurrentPhidotv2.pdf} 
\caption{The rotational speed of the domain wall as a function of applied current. The absolute value of $\langle\dot{\phi}\rangle$ is shown in the plot, the true value is negative.}
...
\end{align}
These emergent electromagnetic fields will then generate a current, which lies in the skyrmion plane and is perpendicular to the external current applied to the material. This therefore accounts for a velocity component in the $y$-direction. However, as the velocity component in the $y$-direction is canceled in the special case $\xi=\alpha$, there must be another force counteracting the Lorentz force. This turns out to be a force dubbed the Magnus force, as an analogy to the force in fluid mechanics under the same name. The vortex-like skyrmion experiences a force perpendicularly to its original motion due to the applied current because the electrons in the current travel faster on one side of the skyrmion than the other. As this deflects the electron flow, meaning there is a force acting upon the conduction electrons, Newton's third law dictates that there is an equal and opposite force acting on the local magnetization of the skyrmion. This is analogous to the Magnus effect in fluid mechanics, where the path of a rotating sphere (like a tennis ball) in a viscous air flow is curved to one side, as the air travels faster on one side than the other, creating a pressure gradient perpendicularly to the air flow. The Magnus force on skyrmions is described in more detail by Everschor in \cite{EverschorDissertation}, on which this explanation was based. 
