...
\end{align}
If the position of the skyrmion core at a time $t$ in the $xy$-plane is given by $\left(\rho_0(t)\cos\phi(t), \rho_0(t)\sin\phi(t)\right)$, the angle $\theta$ can be written as $\theta(\rho-\rho_0(t))$ and the terms $(\hat{j}_e\cdot\nabla)\phi(t)$ vanish as $\phi$ only depends on time. If we apply a uniform current in the $x$-direction, the equations of motion become
\begin{align}
...
\end{align}
In the exception of a special case with $\xi = \alpha$, this will lead to a deformation of the domain wall as the equations of motion are dependent on position. In the special case with $\xi = \alpha$, however, we get $\dot{\phi} = 0$ and the motion of the skyrmion is therefore just a translational motion. As $\cos\phi\hat{\rho} = \hat{x}$, the velocity of the skyrmion becomes $\vec{v} = -b_J\hat{x}$. Remember that $b_J$ is a velocity that scales linearly with the current, defined in \eqref{eq:bJ}. The electrons go in the opposite direction of the electrical current, meaning that the skyrmion follows the motion of the electrons with a velocity $b_J$.\\
