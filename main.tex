...
\subsection{The Rashba effect and the Dzyaloshinskii-Moriya interaction}
...
with $D = 2k_RA$, where we have used that $\vec{A}\cdot(\vec{B}\times\vec{C}) = \vec{B}\cdot(\vec{C}\times\vec{A})$. Remember that there is an implicit sum over $i$. As one can see, the exchange energy density contains a symmetric term like before, and an antisymmetric term that comes from the transformation we had to perform to get rid of the Rashba-term in the Hamiltonian. The antisymmetric term is also known as the Dzyaloshinskii-Moriya interaction, which can also be written as

\begin{align}
\vec{D}_{ij}\cdot(\vec{S}_i\times\vec{S}_j).
\end{align}

The form of the interaction in \eqref{eq:exchEnDensDM} comes from our assumptions of a slowly varying magnetization, making the magnetization a continuous function. This interaction term favors a slight canting of the spins, the direction of which depends on the sign of $D$. The lowest exchange energy of the spins is therefore not when the spins are perfectly aligned or anti-alligned, if this interaction is of a significant order. For this to be the case, the material must have a strong spin-orbit coupling and a broken inversion symmetry.
