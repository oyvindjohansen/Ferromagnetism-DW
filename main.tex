...
\subsubsection{Example: Domain wall advection}
...
This means that $\frac{\delta \epsilon}{\delta \theta} = 0$. Inserting this into \eqref{eq:LLG_current_explicit_spherical} gives us the two equations
...
The final speed of the domain wall is therefore decided by the damping factor $\xi$ of the itinerant spins. Note that this is only true up to a certain critical current, as when the current passes a certain threshold $\dot{\phi} = 0$ is no longer a solution. As $\sin2\phi$ can maximum be 1, we then find that the critical current $j_c$ is given by the relations
...
\begin{align}
\langle\sin2\phi(t)\rangle = \frac{1}{\frac{\pi}{\sqrt{(j_e/j_c)^2-1}}} \int_0^{\frac{\pi}{\sqrt{(j_e/j_c)^2-1}}} \d t \sin2\phi(t) = \frac{j_e}{j_c}-\sqrt{1-(\frac{j_c}{j_e})^2}.
\end{align}
...
\nonumber \langle\dot{X}\rangle &= \frac{\lambda}{T}\left[\frac{1}{\alpha}(\frac{j_e}{j_c}-\sqrt{1-(\frac{j_c}{j_e})^2}) + \frac{1+\xi\alpha}{\xi-\alpha}\cdot\frac{j_e}{j_c}\right] \\
&= \frac{\xi-\alpha}{1+\alpha^2} \left[\frac{1}{\alpha}(\frac{j_e}{j_c}-\sqrt{1-(\frac{j_c}{j_e})^2}) + \frac{1+\xi\alpha}{\xi-\alpha}\cdot\frac{j_e}{j_c}\right] b_{JC}.
\end{align}
