...
In the simplest case when we only have an adiabatic spin-transfer torque ($\xi = \alpha$), \eqref{eq:LLG_current_explicit} can be reduced considerably to
...
in the absence of an external magnetic field. Together, this gives us
...
We then use \eqref{eq:theta_doublediff} and \eqref{eq:theta_current}  to find
...
j_c &= \frac{\alpha(1+\xi^2)}{\xi-\alpha}\frac{\lambda e \gamma ' K_{\perp}}{\mu_0\mu_B P}.
\end{align}
...
\end{subfigure}
\caption{The translational speed of the domain wall with a high damping parameter $\xi$ of the itinerant electrons (a) and without damping of the itinerant electrons (b) with $\alpha = 1$.}
